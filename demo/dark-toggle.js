...
class DarkToggle extends HTMLElement {
    ...
    attributeChangedCallback(name, _oldValue, newValue) {
        if (!this.isConnected)
            return;
        if (name === 'orientation') {
            if (newValue !== 'vertical' && newValue !== 'horizontal' && newValue !== null)
                return;
            this.render();
            this.syncFromDocument();
        }
    ...
    render() {
        ...
        const button = el('button', {
            type: 'button',
            className: `shadow-rim relative inline-flex ${directionClass} cursor-pointer gap-px rounded-full bg-neutral-200/60 dark:bg-neutral-800/50`,
            role: 'switch',
            'aria-checked': 'false',
            ...
            el('span', {
                className: `shadow-edge absolute left-0 top-0 size-8 rounded-full bg-white shadow-md transition-all! delay-0! duration-200! ${indicatorShiftClass} dark:bg-neutral-600/60`,
                'aria-hidden': 'true',
            }),
            el('span', {
                className: 'z-10 flex size-8 items-center justify-center rounded-full text-indigo-700/60 transition-all hover:text-black dark:text-indigo-200/60 dark:hover:text-indigo-200',
                'aria-hidden': 'true',
            }, [sunIcon()]),
            el('span', {
                className: 'z-10 flex size-8 items-center justify-center rounded-full text-indigo-400/60 transition-all hover:text-indigo-700/60 dark:text-indigo-200/60 dark:hover:text-white',
                'aria-hidden': 'true',
            }, [moonIcon()]),
        ]);
